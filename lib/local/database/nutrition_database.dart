...
class NutritionDatabase {
  late ndatabase.NutritionsDao nutritionsDao;
  List<ndatabase.NutritionData> data = [];

  late List<NutritionData> foods = [];

  late List<NutritionData> meals = [];

  late List<NutritionData> ingredients = [];

  NutritionDatabase(this.nutritionsDao);

  void createData() async {
    nutritionsDao.syncFirebaseData();
    data = await nutritionsDao.getAllNutrition();
    for (var element in data) {
      print('This is a data element');
      print(element);
      switch (element.foodType) {
        case 'food':
          foods.add(NutritionData(
              name: element.name,
              calories: element.calories ?? 0.0,
              protein: element.protein ?? 0.0,
              carbohydrates: element.carbohydrates ?? 0.0,
              fat: element.fat ?? 0.0));
          break;
        case 'meal':
          foods.add(NutritionData(
  ...
  //       fat: 16.0),
  //   NutritionData(
  //       name: "Tomato Basil Mozzarella Skewers",
  //       calories: 250.0,
  //       protein: 15.0,
  //       carbohydrates: 10.0,
  //       fat: 18.0),
  ...
  Future<List<NutritionData>> get(FoodType foodType) async {
    List<NutritionData> searchResults = [];
    switch (foodType) {
      case FoodType.food:
        searchResults.addAll(
      ...
      case FoodType.meal:
        ...
        break;
      case FoodType.ingredient:
        searchResults.addAll(ingredients
            .where((ingredient) => ingredient.name.toLowerCase().contains("")));
        break;
    }
    return searchResults;
  }
